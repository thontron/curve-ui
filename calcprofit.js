...
async function getDeposits() {
    ...
    for (const hash of txs) {
        ...
        if(addliquidity.length) {        
            let [yDAI, yUSDC, yUSDT, yTUSD] = (web3provider.eth.abi.decodeParameters(['uint256[4]','uint256[4]', 'uint256', 'uint256'], addliquidity[0].data))[0]
...
async function getWithdrawals(address) {
    ...
    for(let log of logs) {
        ...
        if(removeliquidity.length) {
            let [yDAI, yUSDC, yUSDT, yTUSD] = (web3provider.eth.abi.decodeParameters(['uint256[4]','uint256[4]', 'uint256'], removeliquidity[0].data))[0]
            let yTokens = [yDAI, yUSDC, yUSDT, yTUSD];
    ...
    localStorage.setItem('wlastAddress', default_account)
...
async function init_ui() {
    ...
    try {
        ...
        localStorage.removeItem('wlastAddress')
